...
export function SimpleNodeEditor({
  ...
  // Simple local state for title editing
  ...
  // Sync title value when node changes or editing starts
  useEffect(() => {
    if (node && !isTitleEditing) {
  ...
  }, [isOpen, selectedNodeId, node?.title]);
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <div className="space-y-2">
            <UltraLexicalEditor
              ...
              projectId={projectId}
              onCommit={(value) => {
                console.log('Description committed with simple architecture:', value);
              }}
            />
            <p className="text-xs text-muted-foreground">
              🎯 Simple editor • Zero focus loss • Auto-save
            </p>
          </div>
        </div>
      </div>
