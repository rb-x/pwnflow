...
export function ProjectImportDialog({
  ...
  // Handle initial file and password from export flow
  useEffect(() => {
    if (open && initialFile) {
      setFile(initialFile);
      ...
      if (initialPassword) {
        handlePreview(initialFile, initialPassword);
      } else {
        handlePreview(initialFile, "");
      }
    }
  }, [open, initialFile, initialPassword]);
  ...
  const handlePreview = async (fileToPreview: File, passwordToUse: string) => {
    ...
    try {
      const result = await exportApi.previewProjectImport(
        fileToPreview,
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await exportApi.importProject(
        file,
        password || undefined,
        importMode,
        undefined // target project ID for merge mode - not implemented yet
      );
                ...
                <>
                  <Alert>
                    ...
                    <AlertDescription>
                      ...
                      <ul className="mt-2 text-sm space-y-1">
                        ...
                        {preview.variable_count > 0 && (
                          <li>• {preview.variable_count} variables</li>
                        )}
                        <li>
                          • Exported{" "}
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <option value="merge" disabled>
                        Merge into existing project (Coming soon)
                      </option>
                    </select>
                    <p className="text-xs text-muted-foreground">
                      {importMode === "new"
                        ? "Creates a fresh project with the imported data"
                        : "Merge nodes into an existing project"}
                    </p>
                  </div>
                  ...
                  <Alert>
                    <AlertDescription>
                      ...
                      be assigned new IDs to prevent conflicts and ensure data
                      integrity.
                    </AlertDescription>
                  </Alert>
                </>
