...
def generate_nodes_with_relationships_task(
    ...
    """Generate nodes and relationships using configured AI provider"""
    ...
    try:
        ...
        async def _generate():
            ...
            async with provider as service:
                opts = (
                    AIGenerationOptions(**options) if options else AIGenerationOptions()
                )
                current_task.update_state(
                    state="PROGRESS", meta={"status": "Generating with AI..."}
                )
...
def suggest_connections_task(
    nodes: list, existing_relationships: list = None
) -> dict:
    """Suggest connections between nodes"""
    current_task.update_state(
        state="PROGRESS", meta={"status": "Analyzing connections..."}
    )
